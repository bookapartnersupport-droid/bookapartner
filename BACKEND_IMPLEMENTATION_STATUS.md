...
- No production business data was modified.
\n\n## 2026-09-21 Refund-center hardening\n- Admin Refund Center now reads the real `refunds` table instead of a non-existent booking refund-status field.\n- Refund processing is routed to the protected `payment-refund` Edge Function; Razorpay secrets remain server-side.\n- Pending refund cases can be cancelled through the protected admin refund action.\n- No production business data was modified.\n\n\n## 2026-09-21 Non-payment hardening batch\n- Added missing foreign-key indexes for `action_idempotency.booking_id` and `payment_transactions.customer_id`.\n- Removed duplicate refund uniqueness index.\n- Revoked direct `anon`/`authenticated` execution of the `bap_refresh_partner_rating()` SECURITY DEFINER helper; it remains trigger/backend controlled.\n- Supabase advisor review completed. Remaining advisor items are primarily RLS init-plan/performance tuning, intentionally deferred where changing policy expressions could alter access semantics, plus external provider integrations.\n- No production business data was modified.\n