...
Deno.serve(async(req)=>{
 ...
 if(!tx){const ins=await admin.from("payment_transactions").insert({booking_id:id,customer_id:user.id,amount:Number(bk.total_amount),currency:"INR",status:"pending",provider:"razorpay",metadata:{source:"payment-create-order"}}).select().single();if(ins.error)return json({error:"payment_transaction_create_failed"},500);tx=ins.data;}
